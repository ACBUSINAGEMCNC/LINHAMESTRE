"""Migração para adicionar a coluna blank_laser à tabela item.
...
def migrate_postgres():
    """Adiciona a coluna blank_laser à tabela item no PostgreSQL."""
    conn = None
    try:
        load_dotenv()
        ...
        try:
            ...
            cursor.execute(
                ...
                SELECT EXISTS (
                    ...
                    WHERE table_name = 'item' AND column_name = 'blank_laser'
                );
                """
            )

            if cursor.fetchone()[0]:
                logger.info("Coluna 'blank_laser' já existe na tabela 'item' (PostgreSQL)")
                return True

            cursor.execute(
                """
                ALTER TABLE item
                ADD COLUMN IF NOT EXISTS blank_laser VARCHAR(255);
                """
            )
            logger.info("Coluna 'blank_laser' adicionada com sucesso à tabela 'item' (PostgreSQL)")
            return True
        finally:
            if close_cursor:
                cursor_ctx.close()
...
def migrate_sqlite():
    """Adiciona a coluna blank_laser à tabela item no SQLite."""
    conn = None
    try:
        ...
        if 'blank_laser' in columns:
            logger.info("Coluna 'blank_laser' já existe na tabela 'item' (SQLite)")
            return True

        cursor.execute("ALTER TABLE item ADD COLUMN blank_laser VARCHAR(255)")
        conn.commit()
        logger.info("Coluna 'blank_laser' adicionada com sucesso à tabela 'item' (SQLite)")
        return True
    except Exception as e:
...
def run_migration():
    logger.info("Iniciando migração para adicionar coluna 'blank_laser'...")
