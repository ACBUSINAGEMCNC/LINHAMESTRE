...
def migrate_postgres() -> bool:
    ...
    stmts = [
        "ALTER TABLE estoque_pecas ADD COLUMN IF NOT EXISTS slots_json TEXT",
        "ALTER TABLE estoque_pecas_slot_temp ADD COLUMN IF NOT EXISTS slots_json TEXT",
    ]
...
def migrate_sqlite(db_path: str = None) -> bool:
    ...
    if _table_exists(cur, 'estoque_pecas') and not _has_column('estoque_pecas', 'slots_json'):
        cur.execute("ALTER TABLE estoque_pecas ADD COLUMN slots_json TEXT")
        changed = True

    if _table_exists(cur, 'estoque_pecas_slot_temp') and not _has_column('estoque_pecas_slot_temp', 'slots_json'):
        cur.execute("ALTER TABLE estoque_pecas_slot_temp ADD COLUMN slots_json TEXT")
        changed = True
